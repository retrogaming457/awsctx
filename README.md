...
#### S3-compatible CLI clients:
  ...
  - [s5cmd](https://github.com/peak/s5cmd) is an extremely fast s3 client tool written in Go - dsigned for large-scale S3 operations.   
...
~$ apt install fzf
```
3. Optional: other available S3-compatible CLI clients:
```bash
~$ pip install s3cmd
~$ pip install s4cmd
